...
실패: 9999명
```
- **동시성 제어**: 19,000명 동시 요청 시 1명만 성공 (Optimistic Lock 정상 작동)
- **대용량 트래픽**: 10,000건 요청 처리 (Error 0%, 164.7 TPS)
...
### 동시성 제어 테스트
같은 좌석에 동시 요청 시 단 1명만 성공하는지 검증

| 동시 요청 | 성공 | 실패 | Error % | TPS |
|----------|------|------|---------|-----|
| 10,000명 | 1명 | 9,999명 | 99.99% | 847.4 |
| 19,000명 | 1명 | 18,999명 | 99.99% | 570.3 |

### 대용량 트래픽 테스트
각 사용자가 다른 좌석을 예약하는 실제 서비스 시나리오

| 요청 수 | Ramp-up | Error % | TPS | 평균 응답 |
|--------|---------|---------|-----|----------|
| 10,000건 | 60초 | 0% | 164.7 | 1,309ms |

### 테스트 환경
- 로컬 개발 환경 (Spring Boot 내장 Tomcat)
- MySQL 8.x, Redis (Docker)
- JMeter 5.6.3
